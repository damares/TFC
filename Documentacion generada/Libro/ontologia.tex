  ...
  %\lstinputlisting[language=owlms,literate=,firstnumber=671,firstline=671,lastline=672]{codigo/t-box.ms}
	...
	\begin{description}
		...
		\item[Materia.]La clase \lstinline!Materia! aglutina todos los individuos que representan las diferentes materias de que se compone la titulación. Coloquialmente podemos entender las materias como aquellos individuos que permiten al alumno adquirir competencias, de modo que puedan cumplir con las objetivos generales del título. Formalmente hablando, son materias todos aquellos individuos relacionados con algún individuo de la clase Competencia, y al menos una vez con alguna \lstinline!Competencia_General!. Posteriormente se definirá la propiedad que une \lstinline!Materia! con \lstinline!Compentencia! y con \lstinline!Competencia_General!. 
   		...
   		\lstinputlisting[caption={Descripción de la clase \lstinline!Materia!},label=Materia,captionpos=b,language=owlms,literate=,linerange=Class:\ ects:Materia-only\ ects:Metodo_Docente]{codigo/t-box.ms}	
		...
		Las definiciones de asignatura y de materia son complementarias: Podemos definir una materia como el conjunto de asignaturas de un mismo ámbito, o bien podemos definir una asignatura como parte integrante de una materia, con quien comparte la naturaleza de los conocimientos contenidos. Dado el carácter más general de la materia, se ha optado en la ontología por que sean las materias a partir de las cuales se definan las asignaturas, y no al contrario. 
      	...
      	\todo{Debido a limitaciones del lenguaje, no se tendrán en cuenta limitaciones sobre la extensión de las asignaturas en créditos, limitaciones sobre la multiplicidad de los créditos, ni se hablará acerca de las horas reales a que equivale cada crédito ECTS. 
	...
	Las asignaturas optativas quedan, por el momento, fuera del alcance de este trabajo, por no estar definidas en el documento remitido a ANECA. No obstante, tal y como se verá más adelante, es posible crear incorporar al plan de estudios cualquier asignatura optativa, con todas las características de cualquier asignatura, de modo que la materia ``Optatividad'' esté completa. }
      	...
      	\lstinputlisting[caption={Descripción de la clase \lstinline!Asignatura!},label=Asignatura,captionpos=b,language=owlms,literate=,linerange=Class:\ ects:Asignatura-only\ ects:Caracter]{codigo/t-box.ms}		
...
%		\lstinputlisting[language=owlms,literate=,firstnumber=720,firstline=720,lastline=731]{codigo/t-box.ms}	
	...
	\lstinputlisting[caption={Descripción de la clase \lstinline!Ubicacion_Temporal!},label=ubicacion,captionpos=b,language=owlms,literate=,firstnumber=674,firstline=674,lastline=688]{codigo/t-box.ms}    	
	...
	\lstinputlisting[caption={Descripción de la clase \lstinline!Caracter!},label=caracter,captionpos=b,language=owlms,literate=,firstnumber=626,firstline=626,lastline=640]{codigo/t-box.ms}       	
...
%	Es de destacar que las propiedades únicamente se pueden establecer entre dos individuos. No existen propiedades con cardinalidad tres, lo que implicará que en el caso de que que sea preciso establecer una relación a tres, sería preciso modelarlo como la relación binaria entre el producto escalar de dos de ellas sobre la tercera (tal y como se comentó al mencionar la clase \lstinline!Actividad_Formativa! y la forma de almacenar la cantidad de créditos otorgados).
	...
	\begin{description}
		...
		\lstinputlisting[caption={Descripción de la propiedad \lstinline!OG_seCumpleMedianteLaAdquisicionDe_CO!},label=og-co,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:OG_seCumpleMedianteLaAdquisicionDe_CO-ects:CO_seAdquiereParaCumplir_OG]{codigo/t-box.ms}	
		\lstinputlisting[caption={Descripción de la propiedad \lstinline!CO_seAdquiereParaCumplir_OG!},label=co-og,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:CO_seAdquiereParaCumplir_OG-ects:OG_seCumpleMedianteLaAdquisicionDe_CO]{codigo/t-box.ms}	
    	...
    	\lstinputlisting[caption={Descripción de la propiedad \lstinline!CO_esOtorgadaPor_MA!},label=co-ma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:CO_esOtorgadaPor_MA-ects:MA_otorgaCompetencias_CO]{codigo/t-box.ms}	
		\lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_otorgaCompetencias_CO!},label=ma-co,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:MA_otorgaCompetencias_CO-ects:CO_esOtorgadaPor_MA]{codigo/t-box.ms}	
	    ...
	    \lstinputlisting[caption={Descripción de la propiedad \lstinline!CG_esOtorgadaPor_MA!},label=cg-ma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:CG_esOtorgadaPor_MA-ects:MA_otorgaCompetenciasGenerales_CG]{codigo/t-box.ms}	
	    	\lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_otorgaCompetenciasGenerales_CG!},label=ma-cg,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:MA_otorgaCompetenciasGenerales_CG-ects:CG_esOtorgadaPor_MA]{codigo/t-box.ms}	
	   	...
	   	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    		...
    		\lstinputlisting[caption={Descripción de la propiedad \lstinline!CE_esOtorgadaPor_MA!},label=ce-ma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:CE_esOtorgadaPor_MA-ects:MA_otorgaCompetenciasEspecificas_CE]{codigo/t-box.ms}
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_otorgaCompetenciasEspecificas_CE!},label=ma-ce,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:MA_otorgaCompetenciasEspecificas_CE-ects:CE_esOtorgadaPor_MA]{codigo/t-box.ms}
        ...
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_constaDe_AS!},label=ma-as,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:MA_constaDe_AS-ects:AS_formaParteDe_MA]{codigo/t-box.ms}
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!AS_formaParteDe_MA!},label=as-ma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:AS_formaParteDe_MA-ects:MA_constaDe_AS]{codigo/t-box.ms}
        ...
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_seImparteMediante_ME!},label=ma-as,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:MA_seImparteMediante_ME-ects:ME_utilizadoParaImpartir_MA]{codigo/t-box.ms}
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!ME_utilizadoParaImpartir_MA!},label=me-ma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:ME_utilizadoParaImpartir_MA-ects:MA_seImparteMediante_ME]{codigo/t-box.ms}
        ...
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_seImparteSegun_AF!},label=ma-as,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:MA_seImparteSegun_AF-ects:AF_utilizadaParaImpartir_MA]{codigo/t-box.ms}
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!AF_utilizadaParaImpartir_MA!},label=af-ma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:AF_utilizadaParaImpartir_MA-ects:MA_seImparteSegun_AF]{codigo/t-box.ms}
		...
		\lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_tiene_CA!},label=ma-ca,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:MA_tiene_CA-ects:CA_agrupa_MA]{codigo/t-box.ms}
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!CA_agrupa_MA!},label=ca-ma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:CA_agrupa_MA-ects:MA_tiene_CA]{codigo/t-box.ms}
		...
		\lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_ubicada_UT!},label=ma-ut,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:MA_ubicada_UT-ects:UT_contiene_MA]{codigo/t-box.ms}
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!UT_contiene_MA!},label=ut-ma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:UT_contiene_MA-ects:MA_ubicada_UT]{codigo/t-box.ms}
		...
		\lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_utiliza_MV!},label=ma-mv,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:MA_utiliza_MV-ects:MV_esUtilizadoEn_MA]{codigo/t-box.ms}
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!MV_esUtilizadoEn_MA!},label=mv-ma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:MV_esUtilizadoEn_MA-ects:MA_utiliza_MV]{codigo/t-box.ms}

	    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   		%% AS\_esRequisitoPara\_AS y AS\_tieneComoRequisito\_AS
	   	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    		\item [AS\_esRequisitoPara\_AS y AS\_tieneComoRequisito\_AS.] Como ya se explicó antes, el hecho de que una asignatura tenga como requisito en la guía de la titulación el haber cursado una asignatura anteriormente, no implica que obligatoriamente se haya de cursar esa asignatura con anterioridad. Es decir, se trata de una mera recomendación de cara al itinerario a seguir en la titulación, y en ningún caso de obligado cumplimiento. Se ha tenido en cuenta a la hora de elaborar la ontología, ya que el fin último de esta es proporcionar una herramienta para un diseño de la titulación correcto, lo que incluye el trayecto curricular del alumno, y no para el control del desarrollo del alumno. Por tanto, dado que lo que estamos haciendo es ayudar a diseñar el título, vamos a incluir los requisitos para el curso de asignaturas, como si de cumplimiento obligatorio se tratase.
	    ...
	    \lstinputlisting[caption={Descripción de la propiedad \lstinline!AS_esRequisitoPara_AS!},label=as-as,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:AS_esRequisitoPara_AS-ects:AS_tieneComoRequisito_AS]{codigo/t-box.ms}
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!AS_tieneComoRequisito_AS!},label=as-as2,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:AS_tieneComoRequisito_AS-ects:AS_esRequisitoPara_AS]{codigo/t-box.ms}
	    ...
	    \lstinputlisting[caption={Descripción de la propiedad \lstinline!AS_tiene_CA!},label=as-ca,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:AS_tiene_CA-ects:CA_agrupa_AS]{codigo/t-box.ms}
	    \lstinputlisting[caption={Descripción de la propiedad \lstinline!CA_agrupa_AS!},label=ca-as,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:CA_agrupa_AS-ects:AS_tiene_CA]{codigo/t-box.ms}
	    ...
	    \lstinputlisting[caption={Descripción de la propiedad \lstinline!AS_ubicada_UT!},label=as-ut,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:AS_ubicada_UT-ects:UT_contiene_AS]{codigo/t-box.ms}
		\lstinputlisting[caption={Descripción de la propiedad \lstinline!UT_contiene_AS!},label=ut-as,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:UT_contiene_AS-ects:AS_ubicada_UT]{codigo/t-box.ms}
	    ...
	    \lstinputlisting[caption={Descripción de la propiedad \lstinline!CA_agrupa_ASMA!},label=ca-asma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:CA_agrupa_ASMA-or\ ects:Materia]{codigo/t-box.ms}
        \lstinputlisting[caption={Descripción de la propiedad \lstinline!UT_contiene_ASMA!},label=ut-asma,captionpos=b,language=owlms,literate=,linerange=ObjectProperty:\ ects:UT_contiene_ASMA-or\ ects:Materia]{codigo/t-box.ms}
	...
	\begin{description}
		...
		\item [OG\_Descripcion.] Se trata de una relación entre un individuo de la clase \lstinline!Objetivo\_General! y un dato de tipo literal. Esta relación funcional (un objetivo sólo puede tener una descripción) es útil para guardar la descripción del objetivo descrito, y poder identificarlo por algo menos formal y más útil que la numeración del objetivo.
   		\lstinputlisting[caption={Descripción de la propiedad \lstinline!OG_Descripcion!},label=og,captionpos=b,language=owlms,literate=,linerange=DataProperty:\ ects:OG_Descripcion-xsd:string]{codigo/t-box.ms}
	   ...
	   \item [CO\_Descripcion.] Esta propiedad relaciona una competencia con un literal. También es una propiedad funcional, y al igual que el resto de propiedades dedicadas a almacenar información acerca de descripciones de individuos, es útil para poder identificar los mismos por algo más que el código descriptivo.
   		\lstinputlisting[caption={Descripción de la propiedad \lstinline!CO_Descripcion!},label=co,captionpos=b,language=owlms,literate=,linerange=DataProperty:\ ects:CO_Descripcion-xsd:string]{codigo/t-box.ms}
	    ...
	    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   		...
   		\lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_Creditos!},language=owlms,captionpos=b,label=ma,literate=,linerange=DataProperty:\ ects:MA_Creditos-tipoCredito]{codigo/t-box.ms} 
	   ...
	   \item [MA\_Coordinacion.] Esta ``type property'' se utiliza como un campo libre donde se define de manera informal los agentes que coordinarán la materia, y cuando se defina, las herramentas que utilizarán en su cometido.
		\lstinputlisting[caption={Descripción de la propiedad \lstinline!MA_Coordinacion!},label=ma,language=owlms,captionpos=b,language=owlms,literate=,linerange=DataProperty:\ ects:MA_Coordinacion-xsd:string]{codigo/t-box.ms} 

	    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   		%% ME_Descripcion
	   	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		\item [ME\_Descripcion.] Se trata de una relación entre un individuo de la clase \lstinline!Método\_Docente! y un dato de tipo literal. Esta relación funcional (un método docente sólo puede tener una descripción) es útil para guardar la descripción del método descrito.
   		\lstinputlisting[caption={Descripción de la propiedad \lstinline!ME_Descripcion!},captionpos=b,label=de,language=owlms,literate=,linerange=DataProperty:\ ects:ME_Descripcion-xsd:string]{codigo/t-box.ms}   
		...
		\lstinputlisting[caption={Descripción de la propiedad \lstinline!AF_Creditos!},captionpos=b,label=af,language=owlms,literate=,linerange=DataProperty:\ ects:AF_Creditos-ects:tipoCredito]{codigo/t-box.ms}   
		...
		\item [MV\_Descripcion.] Recoge una pequeña descripción informal del método de evaluación con el que está relacionado, de modo que sea fácil comprobar las pruebas y técnicas de evaluación que el docente utilizará a la hora de verificar el nivel de adquisición de competencias por parte de los alumnos.
   		\lstinputlisting[caption={Descripción de la propiedad \lstinline!MV_Descripcion!},captionpos=b,label=mv,language=owlms,literate=,linerange=DataProperty:\ ects:MV_Descripcion-xsd:string]{codigo/t-box.ms} 
	\end{description}
