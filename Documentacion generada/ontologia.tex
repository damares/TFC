...
\subsection{Individuos de la ontología}
  Los individuos representan objetos de la ontología en el dominio que estamos estudiando. Protegé no hace uso del Unique Name Assumption, es decir, para protegé dos individuos pueden referirse al mismo objeto del mundo real, salvo que se especifique lo contrario. Esta es una consecuencia de las ontologías OWL: todo lo que no sea dicho de forma explícita puede ser cierto. El hecho de que no especifiquemos si dos individuos son o no los mismos, significa que pueden o no serlo, para ese dominio. En nuestra ontología todos los individuos que componen la ontología son distintos, lo que se especificara correctamente en la definición de los individuos.
  
  Las referencias IRI tiene ciertos carácteres reservados por lo que no puedens er utilizados en los nombres de individuos, Por ello, se han evitado los caracteres "/", así como vocales acentuadas, con el fin de evitar que las referencias IRI de los diferentes individuos hagan la misma ilegible.
  
  
  
\begin{description}

 \item [OBJ01 - OBJ11]. Son individuos pertenecientes a la clase \lstinline{Objetivo_General}, que representan los objetivos generales del título, desde el número 1 al 11.
 			 
 		\lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ01-rdfs:Literal]{a-box.ms}

 		\lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ02-rdfs:Literal]{a-box.ms}
 		
 		 \lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ03-rdfs:Literal]{a-box.ms}	
 		 
 		 \lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ04-rdfs:Literal]{a-box.ms}
 		 
 		 \lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ05-rdfs:Literal]{a-box.ms}
 		 
 		 \lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ06-rdfs:Literal]{a-box.ms}
 		 
 		 \lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ07-rdfs:Literal]{a-box.ms}
 		 
 		 \lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ08-rdfs:Literal]{a-box.ms}
 		 
 		 \lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ09-rdfs:Literal]{a-box.ms}
 		 
 		 \lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ10-rdfs:Literal]{a-box.ms}
 		 
 		 \lstinputlisting[language=owlms,literate=,linerange=Individual:\ OBJ11-rdfs:Literal]{a-box.ms}
 		 
 \item [Competencias Generales]
 		 
 \item [Competencias Específicas]
 
 \item [Materias]. Estos individuos representan las diferentes materias de la titulación que estamos modelando.
 
 
 		 \lstinputlisting[language=owlms,literate=,linerange=Individual:\ MA\-Empresa-rdfs:Literal]{a-box.ms} 
 		 
  \item [Asignaturas]
  
  \item [Métodos Docentes]
  
  \item [Actividades Formativas]
  
  \item [Métodos de Evaluacion]
  
  \item [Carácter]
  
  \item [Ubicaciones temporales]
